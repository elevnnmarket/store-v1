...
$com_data = $commission->get_seller_final_order_info($order->get_id(), $seller_id);
...
$result = '
	<div style="margin-bottom: 40px;">
		<p>' . utf8_decode( __( 'You have received an order from', 'marketplace' ) ) . '&nbsp;' . $order->get_formatted_billing_full_name() . '</p>
			...
			if ( ! empty( $fees ) ) {
				foreach ( $fees as $key => $fee ) {
					...
					if($key=='reward-point'){
						...
						if($com_data['wallet_data']){
							$fee_amount = -1 * round(floatval(apply_filters( 'mpmc_get_converted_price', $com_data['wallet_data'])));
						}else{
							continue;
						}
					}else{
					...
					$result .=
						'<tr>
				            <th class="td" scope="row" colspan="2" style="text-align:' . esc_attr( $text_align ) . ';">' . utf8_decode( $fee_name ) . ' : </th>
            				<td class="td">' . wc_price( $fee_amount, array( 'currency' => $order->get_currency() ) ) . '</td>
        				</tr>';
				}
			}
