...
class WC_Stripe_Connect extends WC_Payment_Gateway {
	...
	public function init_form_fields() {
		$this->form_fields = array(
			...
			'title'                       => array(
				...
				'default'           => __( 'Credit Card (Stripe Connect)', 'marketplace-stripe' ),
			),
			'description'                 => array(
			...
			'saveinfo'                    => array(
				'title'       => __( 'Billing Information Storage', 'marketplace-stripe' ),
				'type'        => 'checkbox',
				'label'       => __( 'Allow customers to save billing information for future use (requires Stripe Payment Customer Vault)', 'marketplace-stripe' ),
				'description' => '',
				'default'     => 'no',
			),
		);
	...
	public function admin_options() {
		...
		<p>
		<?php esc_html_e( 'The Stripe Payment Gateway is simple and powerful.  The plugin works by adding credit card fields on the checkout page, and then sending the details to Stripe Payment for verification.', 'marketplace-stripe' ) . '<a href="https://stripe.com/">' . esc_html_e( 'Click here to get paid like the pros.', 'marketplace-stripe' ); ?></a>
		</p>
			...
			<fieldset  style="padding-left: 40px;">
				...
				if ( $this->user_has_stored_data( $user ) ) {
					...
					<fieldset>
						...
						<label for="stripe-use-stored-payment-info-yes" style="display: inline;"><?php esc_html_e( 'Use a stored credit card', 'marketplace-stripe' ); ?></label>
						...
						<fieldset>
							<p>
								...
								<label for="stripe-use-stored-payment-info-no"  style="display: inline;"><?php esc_html_e( 'Use a new payment method', 'marketplace-stripe' ); ?></label>
							</p>
				...
				<p class="form-row form-row-first">
					<label for="ccnum"><?php echo esc_html__( 'Credit Card number', 'marketplace-stripe' ); ?> <span class="required">*</span></label>
				...
				if ( $this->cvv == 'yes' ) {
					...
					<p class="form-row form-row-last">
						...
						<span class="help"><?php esc_html_e( '3 or 4 digits usually found on the signature strip.', 'marketplace-stripe' ); ?></span>
					</p>
					<?php
				}
				?>
				...
				if ( $this->saveinfo == 'yes' && ! ( class_exists( 'WC_Subscriptions_Cart' ) && WC_Subscriptions_Cart::cart_contains_subscription() ) ) {
					...
					<p>
						<label for="saveinfo"><?php esc_html_e( 'Save this billing method?', 'marketplace-stripe' ); ?></label>
						<input type="checkbox" class="input-checkbox" id="saveinfo" name="saveinfo" />
						<span class="help"><?php esc_html_e( 'Select to store your billing information for future use.', 'marketplace-stripe' ); ?></span>
					</p>
					<?php
				}
				?>
				...
				if ( $payment->id == 'stripe-connect' ) {
					...
					<fieldset style="border:1px solid #ddd;padding: 10px;">
						...
						<div class="social-seller-input">
							...
							$stripe_desc = '';
							if ( get_user_meta( get_current_user_ID(), 'wkmppaydesci', true ) ) {
								$stripe_desc = get_user_meta( get_current_user_ID(), 'wkmppaydesci', true );
							}
							?>
							...
							<input type="hidden" name="mp_seller_payment_stripe_method" value="Credit Card (Stripe Connect)">
						</div>

						<div class="mp_seller_paymet_method_description">

							<label for="wkmppaydesci" id="mp_payment_description" name="mp_payment_description"><?php echo esc_html_e( 'Payment Description Or Payment Id' ); ?></label>
							<div class="social-seller-input">
								<input type="text" value="<?php echo isset( $stripe_desc ) ? $stripe_desc : ''; ?>" id="wkmppaydesci" name="wkmppaydesci">
							</div>
	...
	public function save_stripe_seller_payment_details() {
		...
		if ( isset( $postdata['mp_seller_payment_stripe_method'] ) ) {
			$stripe_val = array( 'stripe' => $postdata['mp_seller_payment_stripe_method'] );
		}
		if ( isset( $postdata['wkmppaydesci'] ) ) {
			update_user_meta( get_current_user_ID(), 'wkmppaydesci', $postdata['wkmppaydesci'] );
		}
		if ( isset( $postdata['mp_seller_payment_stripe_method'] ) && isset( $postdata['wkmppaydesci'] ) ) {
			...
			$wpdb->get_results( "update {$wpdb->prefix}mpcommision  set seller_payment_method='" . $payment_methods . "',payment_id_desc='" . $postdata['wkmppaydesci'] . "' where seller_id='" . get_current_user_ID() . "'" );
